...
strain_isotype <- seqs %>% 
  dplyr::select(isotype, strain, prev_names) %>%
  dplyr::filter(!is.na(isotype)) %>%
  dplyr::distinct() %>%
...
strain_isotype <- strain_isotype %>%
  dplyr::arrange(isotype) %>%
  dplyr::select(strain_count, strain, isotype, prev_names)
...
plot_concordance <- function(filename, threshold) {

con <- readr::read_tsv(filename, skip = 1, col_names = c("sample_i", "sample_j", "discordance", "number_of_sites")) %>%
...
iso_set <- dplyr::filter(con, concordance > 0.9995) 

strain_list = unique(c(con$i_strain, con$j_strain))


iso_groups <- tbl_df(stack_list(unique(lapply(strain_list, function(x) {
  ...
  dplyr::left_join(isotypes, by = c("values" = "strain")) %>%
  dplyr::group_by(ind) %>%
  dplyr::mutate(isotype_count = length(unique(isotype))) 
...
a <- ggplot(con) +
  geom_histogram(aes(x=concordance), binwidth = 0.0002) +
...
b <- ggplot(con) +
  geom_histogram(aes(x=concordance, fill = isotype), binwidth = 0.0002) +
  labs(x = "Concordance", y = "Number of Comparisons") +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_manual(values = c("black", "#0080FF")) +
  facet_grid(isotype~., scales = "free_y") +
  geom_vline(aes(xintercept = threshold), color = "red") +
  theme_bw()

z <- ggplot(con) +
  geom_histogram(aes(x=concordance, fill = isotype), binwidth = 0.00002) +
  labs(x = "Concordance", y = "Number of Comparisons") +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(limits = c(0.97, 1.00) ) +
  scale_fill_manual(values = c("black", "#0080FF")) +
  facet_grid(isotype~., scales = "free_y") +
  geom_vline(aes(xintercept = threshold), color = "red") +
  theme_bw()
...
plot_concordance("raw/concordance.tsv", 0.9995)
