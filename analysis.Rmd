...
strain_info <- readr::read_tsv("processed/strain_info.tsv") %>%               dplyr::filter(!is.na(latitude)) %>%
...
leaflet(data = strain_info) %>% 
  addTiles() %>%
  addMarkers(~longitude, ~latitude, popup = paste0(strain, " - ", location, " - ", isolation), 
             icon = ~icons[type] )
...
```{r Concordance}
con <- readr::read_tsv(pipe("grep 'CN' raw/concordance.tsv | awk '{ gsub(\"(# |\\[[0-9]+\\])\",\"\", $0); gsub(\" \",\"_\", $0); print }' | cut -f 2-6 | sed 's/_IND//g'")) %>%
      dplyr::mutate(Concordance = 1-(Discordance/Number_of_sites)) %>%
      dplyr::rename(concordance =Concordance, discordance = Discordance, number_of_sites = Number_of_sites) %>%
      ...
      #dplyr::filter(Number_of_sites > 10000) %>%
      dplyr::filter(!grepl("lane", i_strain)) %>%
      dplyr::filter(!grepl("lane", j_strain))



con <- readr::read_tsv("raw/concordance.tsv", skip = 1, col_names = c("sample_i", "sample_j", "discordance", "number_of_sites")) %>%
      dplyr::mutate(concordance = 1-(discordance/number_of_sites)) %>%
      tidyr::separate(sample_i, sep = "-", into = c("i_run", "i_lib", "i_strain"), extra = "drop", remove = F) %>%
      tidyr::separate(sample_j, sep = "-", into = c("j_run", "j_lib", "j_strain"), extra = "drop", remove = F) 

iso_set <- dplyr::filter(con, concordance > 0.9997) 

strain_list = unique(c(con$i_strain, con$j_strain))
...
ggplot(concordance) +
  geom_histogram(aes(x=Concordance), color = "#191919", binwidth = 0.0002) +
...
df$isotype <- factor(df$isotype, labels = c("Diverged","Isotype"))

ggplot(df) +
  geom_bar(aes(x=Concordance, fill = isotype), binwidth = 0.0002) +
  geom_histogram(aes(x=Concordance - 0.000001, fill = isotype), binwidth = 0.0002) + # Fix to create full density
  labs("Number of comparisons") + 
  scale_fill_manual(values = c("black", "#0080FF")) +
  presentation + 
  facet_grid(isotype~.) + 
  scale_x_continuous(limits = c(0.90,1)) +
  labs(x = "Concordance Among Genotypes", y = "Number of Comparisons") +
  pub_theme
```