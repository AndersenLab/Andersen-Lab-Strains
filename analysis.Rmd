...
# Load Strain Information from labguru 

lg <- readr::read_csv("raw/labguru_export.csv", na = c("", "-")) %>%
  ...
  dplyr::filter(!is.na(strain), Genotype == "wild type")
...
strain_info <- readr::read_tsv("processed/strain_info.tsv") %>%
...
lat_lon_locations <- readr::read_tsv("raw/lat_lon_locations.csv") %>%
...
strain_isotype <- readr::read_tsv("processed/additional_isotypes.tsv") %>%
  ...
  dplyr::left_join(lat_lon_locations) %>%
  dplyr::arrange(isotype) %>%
  dplyr::select(strain, isotype, prev_names, warning_msg, sequenced, everything(), -Genotype, -location) %>%
  dplyr::mutate(prev_names = ifelse(!is.na(prev_names) & prev_names == strain, NA, prev_names)) %>%
...
# Fix N2 Date...
strain_isotype[year(strain_isotype$isolation_date) == 2051 & !is.na(strain_isotype$isolation_date),"isolation_date"] <- parse_date("1951-01-01")
