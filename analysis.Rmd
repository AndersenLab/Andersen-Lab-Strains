...
doc_strain <- load_eav("raw/eav.strain.tsv") %>%
dplyr::filter(ATTR == "depth_of_coverage") %>%
eav_attr("chrom") %>%
dplyr::filter(ATTR == "depth_of_coverage") %>%
dplyr::filter(chrom == "genome")
...
write_tsv(doc_display, path = "processed/depth_of_coverage.strain.tsv")
...
df <- readr::read_tsv("raw/concordance_strain.tsv") %>%
      ...
      dplyr::mutate(abs_concordance = 1- (discordance / 2848047)) %>%
      # Remove low coverage strains.
      dplyr::filter(number_of_sites > 1E6) %>%
      dplyr::mutate(isotype = (abs_concordance > 0.9993)) # %>%
      #dplyr::filter(sample_i %in% strain_set$strain, sample_j %in% strain_set$strain)
...
single_strains <- strain_list[!(strain_set %in% unique(c(iso_set$sample_i, iso_set$sample_j)))]
single_strains <- as.data.frame(list(sample_i = single_strains, sample_j = single_strains, discordance = 0, number_of_sites = 100, abs_concordance = 1, isotype = TRUE))
...
curr_isotypes %>% readr::write_tsv("processed/20160901.iso_groups.tsv")
