...
concordance <- readr::read_tsv(pipe("grep 'CN' raw/concordance.tsv | awk '{ gsub(\"(# |\\[[0-9]+\\])\",\"\", $0); gsub(\" \",\"_\", $0); print }' | cut -f 2-6 | sed 's/_IND//g'")) %>%
...
iso_set <- dplyr::filter(concordance, Concordance > 0.99) 

attach(concordance)

strain_list = unique(c(i_strain, j_strain))
