...
strain_isotype <- strain_isotype %>%
  dplyr::arrange(isotype) %>%
  dplyr::select(strain_count, strain, isotype, prev_names, warning_msg, sequenced) %>%
...
# Write joined strain info table.
readr::read_tsv("processed/strain_info.tsv") %>%
                        dplyr::full_join(strain_isotype) %>%
                        dplyr::mutate(sequenced = ifelse(is.na(sequenced), F, sequenced)) %>%
                        dplyr::select(strain, isotype, everything(), -strain_count) %>%
                        dplyr::group_by(isotype) %>%
                        dplyr::arrange(strain, isotype) %>%
                        readr::write_tsv("processed/strain_info_join.tsv")
